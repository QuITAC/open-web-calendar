...
DHTMLX_LANGUAGES_FILE = os.path.join(
...
def get_calendar(type):
    ...
    raise ValueError(
        "Cannot use extension {}. Please see the documentation or report an error.".format(
            type
        )
    )
...
def serve_configuration():
    return make_js_file_response(
        "/* generated */\nconst configuration = {};".format(
            json.dumps(get_configuration())
        )
    )
